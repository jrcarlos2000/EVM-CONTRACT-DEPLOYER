...
async function fund(taskArguments, hre) {
  ...
  const localSigners = (await hre.ethers.getSigners()).slice(0,firstLocalAcountsToFund).map((item) => {return item.address});
  ...
  for(let i=0;i<tokensToFund.length;i++){
      ...
      :
      {
        ...
        decimals: decimals[fundNetwork][tokensToFund[i]] || decimals[fundNetwork]['DEFAULT'],
        forkSigner: await getBestHolderSigner(tokensToFund[i], fundNetwork, hre),
      }
